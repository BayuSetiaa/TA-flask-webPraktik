    ...
    def get_all(self, page_num: int = None, item_per_page: int = None, paginate: bool = True) -> list:
        ...
        query = f'''
            SELECT 
                college.code, 
                college.name, 
                COUNT(DISTINCT course.code) AS courses, 
                COUNT(DISTINCT enrolled.student) as enrolled
            FROM college
            LEFT JOIN course ON college.code = course.college
            LEFT JOIN (
                SELECT collegecode, COUNT(*) as student
                FROM students
                GROUP BY collegecode
            ) AS enrolled ON college.code = enrolled.collegecode
