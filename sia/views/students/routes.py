...
from sia.models.student import Student
from sia.models.course import Course
from sia.models.college import College
from . import student
...
def add() -> str:
    if request.method == 'POST':
        ...
        if added:
            flash(f'{student["firstname"]} berhasil ditambahkan!', 'success')
        else:
            flash(f'{student["firstname"]} tidak dapat ditambahkan. Pastikan ID tersebut unik.', 'info')
        return redirect(url_for('student.students'))
    else:
        courses = Course().get_all(paginate=False)
        colleges = College().get_all(paginate=False)
...
def update(id: str) -> str:
    if request.method == 'POST':
        ...
        flash(f"{student['firstname']}'s data telah berhasil diubah!", 'info')
        return redirect(url_for('student.students'))
    else:
        return redirect(url_for('student.students'))
...
def delete(id: str) -> str:
    ...
    delete_image(id)
    Student().delete(id)
    flash(f'{data[0]} dihapus dari database.', 'info')
    return redirect(url_for('student.students'))
