...
def delete(id: str) -> str:
    try:
        College().delete(id)
        flash(f'{id} dihapus dari database.', 'info')
        return redirect(url_for('college.colleges'))
    except:
        flash(f'{id} tidak dapat dihapus. Mahasiswa atau mata kuliah terdaftar di bawah perguruan tinggi yang dipilih.', 'info')
        return redirect(url_for('college.colleges'))
...
def update(id: str) -> str:
    if request.method == 'POST':
        ...
        flash(f"{id} telah berhasil diperbarui!", 'info')
        return redirect(url_for('college.colleges'))
    else:
        return redirect(url_for('college.colleges'))